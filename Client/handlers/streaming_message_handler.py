...
class StreamingMessageHandler:
    ...
    def __init__(self, audio_manager: AudioManager):
        ...
        # 早期TTS触发控制
        self._dialogue_content = ""  # 对话内容部分
        self._has_triggered_early_tts = False  # 是否已触发早期TTS
        self._dialogue_complete = False  # 对话内容是否完整
        
        print("🎵 StreamingMessageHandler初始化完成 - WAV流式音频优化版本 + 早期TTS触发（包含语气）")
    ...
    async def _handle_text_message(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if content:
                ...
                # 检查是否需要触发早期TTS
                await self._check_and_trigger_early_tts(content)
    ...
    async def _check_and_trigger_early_tts(self, content: str):
        """检查是否需要触发早期TTS"""
        if self._has_triggered_early_tts or self._dialogue_complete:
            return
        
        # 更新对话内容
        self._dialogue_content += content
        
        # 检查是否遇到语气描述结束的标志
        # 根据格式：核心对话内容 + 可爱后缀 + (语气描述) + [肢体动作描述] + <面部表情描述> + <<心情描述>>
        # 我们需要检测 ')' 字符，这标志着语气描述结束，此时包含对话内容+语气
        if '(' in self._dialogue_content and ')' in self._dialogue_content and not self._has_triggered_early_tts:
            # 检查是否有完整的语气描述 (xxx)
            paren_start = self._dialogue_content.find('(')
            paren_end = self._dialogue_content.find(')', paren_start)
            
            if paren_start != -1 and paren_end != -1:
                # 提取对话内容+语气部分（包含到第一个完整的括号）
                dialogue_with_mood = self._dialogue_content[:paren_end + 1].strip()
                
                if dialogue_with_mood and len(dialogue_with_mood) > 10:  # 确保有足够的内容
                    print(f"🎵 检测到语气描述结束，触发早期TTS（包含语气）: '{dialogue_with_mood[:50]}...'")
                    
                    # 标记已触发早期TTS
                    self._has_triggered_early_tts = True
                    self._dialogue_complete = True
                    
                    # 触发早期TTS回调
                    if "early_tts" in self.message_callbacks:
                        try:
                            await self.message_callbacks["early_tts"](dialogue_with_mood)
                            print(f"✅ 早期TTS回调成功执行: '{dialogue_with_mood[:30]}...'")
                        except Exception as e:
                            print(f"❌ 早期TTS回调执行失败: {e}")
    ...
    async def _handle_done_message(self, message: Dict[str, Any]) -> bool:
        ...
        # 如果有累积的文本内容，但没有触发早期TTS，调用常规文本完成回调
        if self.current_text_content and not self._has_triggered_early_tts and "text_complete" in self.message_callbacks:
            try:
    ...
    def get_stream_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dialogue_content": self._dialogue_content,
            "has_triggered_early_tts": self._has_triggered_early_tts,
            "dialogue_complete": self._dialogue_complete
        }
    ...
    def reset(self):
        ...
        # 重置早期TTS相关状态
        self._dialogue_content = ""
        self._has_triggered_early_tts = False
        self._dialogue_complete = False
